...
async function handleFindMatch() {
  ...
  try {
    ...
    resultDiv.innerHTML = `
      <div class="match-card" style="padding: 20px; background: linear-gradient(135deg, var(--accent-light), var(--accent)); color: #000000; border-radius: var(--radius); margin-bottom: 16px;">
        <h3 style="margin: 0 0 8px 0; color: #000000;">Best Match Found!</h3>
        <div style="display: flex; align-items: center; gap: 12px; margin: 12px 0;">
          <p style="font-size: 32px; font-weight: 700; margin: 0; color: #000000;">${bestMatch.score} points</p>
        ...
        <p style="margin: 0; color: #000000;"><strong>${window.UI.escapeHtml(bestMatch.user.name || bestMatch.user.username)}</strong></p>
      </div>
      <div style="padding: 16px; background: transparent; border-radius: var(--radius); margin-bottom: 16px;">
        <h4 style="color: #FFFFFF; margin-bottom: 12px;">Why you match:</h4>
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
        ...
        <button id="rematch-btn" style="padding: 14px; font-size: 16px; font-weight: 600; background: var(--accent); color: #000000; border: none; border-radius: var(--radius); cursor: pointer;">
          🔄 Try Another Match
        </button>
      </div>
    `;
...
function showNextMatch() {
  ...
  resultDiv.innerHTML = `
    <div class="match-card" style="padding: 20px; background: linear-gradient(135deg, var(--accent-light), var(--accent)); color: #000000; border-radius: var(--radius); margin-bottom: 16px;">
      <h3 style="margin: 0 0 8px 0; color: #000000;">Match #${AppState.currentMatchIndex + 1}</h3>
      <div style="display: flex; align-items: center; gap: 12px; margin: 12px 0;">
        <p style="font-size: 32px; font-weight: 700; margin: 0; color: #000000;">${match.score} points</p>
        <span style="background: ${qualityBadge.color}; padding: 6px 12px; border-radius: 20px; font-size: 14px; font-weight: 600; color: white;">
          ${qualityBadge.emoji} ${qualityBadge.text}
        </span>
      </div>
      <p style="margin: 0; color: #000000;"><strong>${window.UI.escapeHtml(match.user.name || match.user.username)}</strong></p>
    </div>
    <div style="padding: 16px; background: transparent; border-radius: var(--radius); margin-bottom: 16px;">
      <h4 style="color: #FFFFFF; margin-bottom: 12px;">Why you match:</h4>
      <ul style="line-height: 1.8; color: #FFFFFF;">
...
function displayPreQuizResults(profile) {
  ...
  scoreDiv.innerHTML = `
    <div style="text-align: center; padding: 24px; background: linear-gradient(135deg, var(--accent-light), var(--accent)); color: #000000; border-radius: var(--radius); margin-bottom: 16px;">
      <h2 style="font-size: 48px; margin: 0; color: #000000;">${profile.preQuizScore}%</h2>
      <p style="margin: 8px 0 0 0; color: #000000;">Previous Score: ${profile.preQuizCorrect} / ${profile.preQuizTotal}</p>
      <p style="font-size: 12px; color: #1A1A1A; margin: 4px 0 0 0;">
...
async function handleRematchRequest(currentMatch) {
  try {
    ...
    resultDiv.innerHTML = `
      ...
      <div class="match-card" style="padding: 20px; background: linear-gradient(135deg, var(--accent-light), var(--accent)); color: #000000; border-radius: var(--radius); margin-bottom: 16px;">
        <h3 style="margin: 0 0 8px 0; color: #000000;">Next Match</h3>
        <div style="display: flex; align-items: center; gap: 12px; margin: 12px 0;">
          <p style="font-size: 32px; font-weight: 700; margin: 0; color: #000000;">${bestRematch.score} points</p>
          <span style="background: ${qualityBadge.color}; padding: 6px 12px; border-radius: 20px; font-size: 14px; font-weight: 600; color: white;">
            ${qualityBadge.emoji} ${qualityBadge.text}
          </span>
        </div>
        <p style="margin: 0; color: #000000;"><strong>${window.UI.escapeHtml(bestRematch.user.name || bestRematch.user.username)}</strong></p>
      </div>
      <div style="padding: 16px; background: transparent; border-radius: var(--radius); margin-bottom: 16px;">
        <h4 style="color: #FFFFFF; margin-bottom: 12px;">Why you match:</h4>
        <ul style="line-height: 1.8; color: #FFFFFF;">
